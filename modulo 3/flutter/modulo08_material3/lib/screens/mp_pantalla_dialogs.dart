...
class MpPantallaDialogs extends StatelessWidget {
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Estas seguro de que deseas eliminar a Max?\n'
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Mascota "${ctrlNombre.text}" agregada')),
      );
    }
  }
  ...
  Future<void> _mostrarAlertDialogBasico(BuildContext context) async {
    await showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Ejemplo en clase alert dialog.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title:           const Text('SnackBar y Dialog - Vet AP'),
      ...
      body: ListView(
        ...
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Eliminar mascota (confirmacion)'),
          ),
          const SizedBox(height: 8),
          FilledButton.tonal(
            onPressed: () => _mostrarFormulario(context),
            child: const Text('Agregar mascota (formulario)'),
          ),
          const SizedBox(height: 12),
          FilledButton(
